...
df_train <- df_train %>% mutate(brand_agg = if_else(brand %in% luxurious_brands$brand,"Luxurious","Not Luxurious"))
```
...
```{r}
df_test <- df_test %>% mutate(brand_agg = if_else(brand %in% luxurious_brands$brand,"Luxurious","Not Luxurious"))
```
...
write_csv(linear_preds, file="predictions9.csv")
```
...
write_csv(rf_preds, file="predictions8.csv")
```
...
write_csv(xgb_preds, file="predictions7.csv")
```
