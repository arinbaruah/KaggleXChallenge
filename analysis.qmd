...
```{r}
ggplot(data = df_train %>% filter(price < 50000),aes(y = brand,x = price)) + geom_boxplot()
```
...
df_test <- df_test %>% mutate(brand_general= if_else(brand %in% luxurious_brands$brand,"General","Not General"))
```
...
write_csv(linear_preds, file="predictions17.csv")
```
...
write_csv(rf_preds, file="predictions10.csv")
```
...
write_csv(xgb_preds, file="predictions12.csv")
```


```{r}
preds <- tibble(id = df_train$id,
                pred_price =  predict(linear_model,new_data=df_train),
                actual_price = df_train$price)

ggplot(data = preds,aes(x = pred_price,y = actual_price)) + geom_point() + geom_smooth(method = "lm")
```
