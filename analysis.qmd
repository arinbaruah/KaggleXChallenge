...
df_train <- df_train %>% mutate(premiumness = case_when(price > 10^6 ~ "Luxurious",
                                                        price < 10^6 & price >250000 ~ "Premium",
                                                        price > 20000 & price <= 250000 ~ "Mid range",
...
rf_model <- randomForest(price ~ ., data = df_train[,-1], ntree = 500, 
                         mtry = 3,min_n = 5, importance = TRUE)
```
...
write_csv(rf_preds, file="predictions35.csv")
```
...
write_csv(xgb_preds, file="predictions35.csv")
```
...
preds <- tibble(id = df_train$id,
                pred_price =  predict(lgb_model,new_data=df_train)$.pred,
...
write_csv(lgb_preds, file="predictions36.csv")
```