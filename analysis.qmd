...
```{r}
ggplot(data = df_train,aes(y = Engine_spec2,x = price)) + geom_point()
```
...
df_train <- df_train %>% mutate(premiumness = case_when(price > 10^6 ~ "Exotic",
                                                        price < 10^6 & price > 500000 ~ "Luxurious",
                                                        price > 150000 & price <= 500000 ~ "Premium",
                                                        price > 80000 & price <= 150000 ~ "Upper Mid range",
                                                        price > 20000 & price <= 80000 ~ "Mid range",
                                                        
                                                        price <= 20000 ~ "Budget"))

luxurious_brands <- df_train %>% filter(premiumness == "Luxurious") %>% select(brand)
budget_brands <- df_train %>% filter(premiumness == "Budget") %>% select(brand)
premium_brands <- df_train %>% filter(premiumness == "Premium") %>% select(brand)


df_train <- df_train %>% mutate(brand_budget = if_else(brand %in% budget_brands$brand,1,0))
df_train <- df_train %>% mutate(brand_premium = if_else(brand %in% premium_brands$brand,1,0))
#df_train <- df_train %>% mutate(brand_luxurious = if_else(brand %in% luxurious_brands$brand,1,0))
...
```{r}
#df_train <- df_train %>% filter(Engine_spec1 > 0)
...
df_test <- df_test %>% mutate(brand_budget = if_else(brand %in% budget_brands$brand,1,0))
df_test <- df_test %>% mutate(brand_premium = if_else(brand %in% premium_brands$brand,1,0))
#df_test <- df_test %>% mutate(brand_luxurious = if_else(brand %in% luxurious_brands$brand,1,0))
...
corrplot(cor(df_train %>% select_if(is.numeric)),        # Correlation matrix
...
write_csv(linear_preds, file="predictions43.csv")
```
...
set.seed(123)
rf_model <- randomForest(price ~ ., data = df_train[,-1], ntree = 500, 
                         mtry = 3,min_n = 7, importance = TRUE)
```
...
rf_preds <- rf_preds %>% mutate(across(price, ~replace_na(., mean(., na.rm=TRUE))))

write_csv(rf_preds, file="predictions37.csv")
```
...
write_csv(xgb_preds, file="predictions45.csv")
```
...
ggplot(data = preds,aes(x = pred_price,y = actual_price)) + geom_point() + geom_smooth() + coord_fixed(ratio = 1) + ggtitle(paste0("RMSE:",round(rmse_vec(preds$actual_price,preds$pred_price),2)))
```
...
lgb_preds <- tibble(id = df_test$id,
  price = predict(lgb_model,new_data=df_test)$.pred)

lgb_preds <- lgb_preds %>% mutate(across(price, ~replace_na(., median(., na.rm=TRUE))))

write_csv(lgb_preds, file="predictions40.csv")
```
