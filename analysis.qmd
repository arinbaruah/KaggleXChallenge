...
```{r}
df_train <- df_train %>% select(-c(engine,premiumness,Engine_spec9,Engine_spec8,Engine_spec7,Engine_spec4,clean_title))
df_train
```


```{r}
linear_model <- lm(price~ .,data = df_train[,-1] %>% drop_na())
```
...
```{r}
df_test <- df_test %>% select(-c(engine,Engine_spec9,Engine_spec8,Engine_spec7,Engine_spec4,Engine_spec8,clean_title))
df_test
```
...
```{r}
predict(linear_model,newdata=df_test %>% filter() %>% drop_na())
```
